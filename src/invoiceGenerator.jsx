...
const InvoiceTemplateCreator = () => {
  ...
  return (
    <div className="invoice-container">
      {/* Editor Panel */}
      <div className={`editor-panel ${mode === 'edit' ? 'editor-panel-edit' : 'editor-panel-preview'}`}>
        ...
        <div className="editor-content">
              ...
              <div className="section-content-spaced">
                ...
                <div>
                  ...
                  {AVAILABLE_FIELDS.header.topRow.map(field => {
                    ...
                    return (
                      <div key={field.id} className={`field-item ${isActive ? 'field-item-active' : 'field-item-inactive'}`}>
                        <input
                          type="checkbox"
                          checked={isActive}
                ...
                <div className="option-box">
                  <label className="option-label">
                    <input
                      ...
                      className="field-checkbox"
                    />
                    <span className="option-text">Show Ct.Size under SKU name</span>
                  </label>
                </div>
          ...
          <div className="settings-container">
            ...
            <div className="settings-checkbox-group">
              ...
              />
              <label className="settings-checkbox-label">Show Table Borders</label>
            </div>
          </div>
        </div>
        ...
        <div className="toolbar">
          <div className="toolbar-group">
            <button
              ...
              className={`toolbar-button ${mode === 'edit' ? 'toolbar-button-active' : 'toolbar-button-inactive'}`}
            >
              <Edit size={18} />
              Edit
            </button>
            <button
              ...
              className={`toolbar-button ${mode === 'preview' ? 'toolbar-button-active' : 'toolbar-button-inactive'}`}
            >
              <Eye size={18} />
              Preview
            </button>
                  ...
                  {template.header.left.map(fieldId => {
                    ...
                    return (
                      <div key={fieldId} className="invoice-header-field">
                        <span className="invoice-header-label">{field.label}: </span>
              ...
              <table className="invoice-table">
                <thead>
                  <tr className="invoice-table-header">
                    <th className={`invoice-table-header-cell ${template.showTableBorders ? 'invoice-table-header-cell-bordered' : ''}`}>#</th>
                    <th className={`invoice-table-header-cell ${template.showTableBorders ? 'invoice-table-header-cell-bordered' : ''}`} style={{ minWidth: '120px' }}>SKU</th>
                    {template.lineItems.filter(f => f !== 'sku').map(fieldId => {
                      const field = AVAILABLE_FIELDS.lineItems.find(f => f.id === fieldId);
                      return (
                        <th
                          key={fieldId}
                          className={`invoice-table-header-cell invoice-table-header-cell-right ${template.showTableBorders ? 'invoice-table-header-cell-bordered' : ''}`}
                          style={{ minWidth: '50px' }}
                        >
                          {field.label}
                        </th>
                ...
                <tbody>
                  {PLACEHOLDER_DATA.lineItems.map((item, idx) => (
                    <tr key={idx} className={template.showTableBorders ? '' : 'invoice-table-row'}>
                      <td className={`invoice-table-cell ${template.showTableBorders ? 'invoice-table-cell-bordered' : ''}`}>{idx + 1}.</td>
                      <td className={`invoice-table-cell invoice-sku-cell ${template.showTableBorders ? 'invoice-table-cell-bordered' : ''}`}>
                        <div className="invoice-sku-name">{item.sku}</div>
                        {template.showCtSize && item.ctSize && (
                          <div className="invoice-sku-size">* Ct.Size ({item.ctSize})</div>
                        )}
                      </td>
                      {template.lineItems.filter(f => f !== 'sku').map(fieldId => {
                        const field = AVAILABLE_FIELDS.lineItems.find(f => f.id === fieldId);
                        ...
                        return (
                          <td
                            key={fieldId}
                            className={`invoice-table-cell invoice-table-cell-right ${template.showTableBorders ? 'invoice-table-cell-bordered' : ''}`}
                          >
                            {field.type === 'number' ? (
                        ...
                        {template.summary.map(fieldId => {
                          ...
                          return (
                            <tr key={fieldId} className="invoice-summary-row">
                              <td className="invoice-summary-label">{field.label}</td>
